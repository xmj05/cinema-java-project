...
public class cinema {
	...
	public void update_board(int argu, int argu2) {
		...
		outerloop: for (int i = 0; i < board_size; i++) {
			for (int j = 0; j < board_size; j++) {
				temp = (List) board.get(i);
	...
	public void buy_tickets() {
		...
		for (int i = 0; i != no2 + 1; i++) {
			...
			System.out.println("please choose your row(A to I) for ticket " + Integer.toString(i + 1));
			String horizontal2 = (horizontal.nextLine()).toLowerCase();
			char horizontal3 = horizontal2.charAt(0);
			int horizontal4 = (int) horizontal3 - 97;
			Scanner vertical = new Scanner(System.in);
			System.out.println("please choose your column(1 to 10) for ticket " + Integer.toString(i + 1));
			int vertical2 = vertical.nextInt() - 1;
			System.out.println(vertical2);
			System.out.println(horizontal4);
			x.update_board(vertical2, horizontal4);
		}
		if (status) {
